...
func (store *RuntimeDurableStore) StoreAssignmentPayload(payload DurableAssignmentPayload) error {
	store.mu.Lock()
...
func (store *RuntimeDurableStore) storeAssignmentPayloadLocked(payload DurableAssignmentPayload) error {
...
func (store *RuntimeDurableStore) AssignmentPayload(attemptID string) (DurableAssignmentPayload, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
...
func (store *RuntimeDurableStore) loadAssignmentSpool() error {
...
func (store *RuntimeDurableStore) reconcileAssignmentPayloadsLocked() error {
...
func (store *RuntimeDurableStore) assignmentPayloadPath(attemptID, assignmentMessageID string) string {
