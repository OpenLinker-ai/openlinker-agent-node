...
func (store *RuntimeDurableStore) SpoolUsage() (RuntimeSpoolUsage, error) {
	store.mu.Lock()
...
func (store *RuntimeDurableStore) AcceptsNewRuns() bool {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *RuntimeDurableStore) spoolUsageLocked() RuntimeSpoolUsage {
...
func (store *RuntimeDurableStore) acceptsNewRunsLocked() bool {
...
func (store *RuntimeDurableStore) ensureSpoolWriteLocked(path string, size int64) error {
...
func (store *RuntimeDurableStore) trackSpoolFileLocked(path string, size int64) error {
	if size <= 0 {
...
func (store *RuntimeDurableStore) removeSpoolFileLocked(path string) error {
...
func (store *RuntimeDurableStore) setSpoolLimitsForTest(maximumBytes, maximumRecords, reserveBytes int64) error {
	store.mu.Lock()
	defer store.mu.Unlock()
