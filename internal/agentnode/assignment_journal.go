...
// every journal, Event, and Result record to the authenticated Node/Agent and
...
// RuntimeDurableStore owns the data-directory process lock for its lifetime.
// All mutation methods serialize locally; the OS lock excludes other Node
...
func OpenRuntimeDurableStore(dataDir string) (_ *RuntimeDurableStore, retErr error) {
...
func (store *RuntimeDurableStore) Identity() RuntimeIdentity {
	store.mu.Lock()
	defer store.mu.Unlock()
	return store.identity
}

func (store *RuntimeDurableStore) Close() error {
	store.mu.Lock()
	defer store.mu.Unlock()
	if store.closed {
		return nil
...
func (store *RuntimeDurableStore) CreateAssignment(record AssignmentJournalRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
...
func (store *RuntimeDurableStore) AdvanceAssignment(assignmentMessageID string, next AssignmentState) (AssignmentJournalRecord, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
...
func (store *RuntimeDurableStore) Assignment(assignmentMessageID string) (AssignmentJournalRecord, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
		return AssignmentJournalRecord{}, err
...
func (store *RuntimeDurableStore) Assignments() ([]AssignmentJournalRecord, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
		return nil, err
...
func (store *RuntimeDurableStore) DeleteAssignment(assignmentMessageID string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
		return err
...
func (store *RuntimeDurableStore) openAndReplayJournal() error {
...
func (store *RuntimeDurableStore) loadJournalSnapshot() (*assignmentJournalSnapshot, error) {
...
func (store *RuntimeDurableStore) appendJournalLocked(entry assignmentJournalEntry) error {
	if err := store.readyLocked(); err != nil {
		return err
	}
...
func (store *RuntimeDurableStore) applyJournalEntryLocked(entry assignmentJournalEntry, replay bool) error {
	record := entry.Record
	if err := record.Identity.validate(); err != nil {
		return err
	}
...
func (store *RuntimeDurableStore) writeJournalSnapshotLocked() error {
...
func (store *RuntimeDurableStore) setDurableHookForTest(hook durableHook) {
	store.mu.Lock()
	defer store.mu.Unlock()
	store.hook = hook
}

func (store *RuntimeDurableStore) assignmentForAttemptLocked(attemptID string) (assignmentJournalEntry, error) {
