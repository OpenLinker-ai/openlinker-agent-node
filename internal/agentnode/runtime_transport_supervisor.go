...
func (node *Node) startInitialRuntimeTransport(parent context.Context) (*openlinker.RuntimeV2ReadyPayload, error) {
...
func (node *Node) startTransportSupervisor() {
...
func (node *Node) transportSupervisorLoop(ctx context.Context) {
	probeAttempt := 0
	for ctx.Err() == nil {
		kind, _, active := node.transport.snapshot()
		switch kind {
		case RuntimeTransportWebSocket:
			connection, ok := active.(RuntimeV2DuplexClient)
			if !ok {
		...
		case RuntimeTransportPull:
			...
			err := node.RuntimeDialer.ProbeRuntimeV2WebSocket(probeCtx)
			cancel()
			if err != nil {
...
func (node *Node) switchToPull(ctx context.Context) error {
...
func (node *Node) switchToWebSocket(ctx context.Context) error {
	...
	// before returning so the Node never remains transportless after a failed
	// recovery probe.
...
func (node *Node) reconnectWebSocket(ctx context.Context) error {
...
func (node *Node) activatePullWithRetry(parent context.Context, reconnect bool) (*openlinker.RuntimeV2ReadyPayload, error) {
...
func (node *Node) attachPullWithRetry(parent context.Context, reconnect bool) (*openlinker.RuntimeV2ReadyPayload, error) {
...
func (node *Node) activateWebSocketWithRetry(parent context.Context, reconnect bool) (*openlinker.RuntimeV2ReadyPayload, error) {
...
func (node *Node) dialWebSocketOnce(parent context.Context) (RuntimeV2DuplexClient, *openlinker.RuntimeV2ReadyPayload, error) {
	...
	connection, err := node.RuntimeDialer.DialRuntimeV2WebSocket(callCtx, node.runtimeHello())
	cancel()
	if err != nil {
		return nil, nil, err
	}
	ready, err := connection.CreateRuntimeV2Session(parent, node.runtimeHello())
...
func (node *Node) closeTransport(client RuntimeV2Client, reason string) {
	if client == nil || node.store == nil {
		return
	}
	identity := node.store.Identity()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	_ = client.CloseRuntimeV2Session(ctx, openlinker.RuntimeV2SessionCloseRequest{
