...
func TestRuntimeV2ClientUsesTLS13MTLSAndExactInvocationProof(t *testing.T) {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		case "/api/v1/agent-runtime/v2/sessions":
			...
			var hello openlinker.RuntimeV2HelloPayload
			...
			_ = json.NewEncoder(w).Encode(openlinker.RuntimeV2ReadyPayload{
		...
		case "/api/v1/agent-runtime/v2/call-agent":
			...
			var delegated openlinker.RuntimeV2CallAgentRequest
			...
			_ = json.NewEncoder(w).Encode(openlinker.RuntimeV2RunSummary{
	...
	if _, err := client.CreateRuntimeV2Session(context.Background(), hello); err != nil {
		t.Fatalf("create mTLS runtime session: %v", err)
	}
	summary, err := client.CallRuntimeV2Agent(context.Background(), openlinker.RuntimeV2CallAgentAuthorization{
		NodeEnvelope:         nodeEnvelope,
		AgentInvocationToken: invocation,
		IdempotencyKey:       idempotencyKey,
	}, openlinker.RuntimeV2CallAgentRequest{
