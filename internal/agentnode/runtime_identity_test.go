...
func TestRuntimeDataDirLockRejectsSecondOpen(t *testing.T) {
	...
	_, err := OpenRuntimeDurableStore(dataDir)
...
func TestRuntimeDataDirProcessLock(t *testing.T) {
	if os.Getenv("OPENLINKER_TEST_LOCK_HELPER") == "1" {
		store, err := OpenRuntimeDurableStore(os.Getenv("OPENLINKER_TEST_LOCK_DIR"))
		if err != nil {
	...
	if _, err := OpenRuntimeDurableStore(dataDir); !errors.Is(err, ErrDataDirLocked) {
		_ = stdin.Close()
		_ = command.Wait()
...
func openRuntimeStoreForTest(t *testing.T, dataDir string) *RuntimeDurableStore {
	t.Helper()
	store, err := OpenRuntimeDurableStore(dataDir)
	if err != nil {
...
func persistStartedAssignmentForTest(t *testing.T, store *RuntimeDurableStore, suffix string) AttemptIdentity {
	t.Helper()
