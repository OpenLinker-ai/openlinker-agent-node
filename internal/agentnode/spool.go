...
func (store *RuntimeDurableStore) AppendEvent(identity AttemptIdentity, eventType string, payload json.RawMessage) (EventSpoolRecord, error) {
	store.mu.Lock()
...
func (store *RuntimeDurableStore) StoreEvent(record EventSpoolRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
		return err
...
func (store *RuntimeDurableStore) storeEventLocked(record EventSpoolRecord) error {
...
func (store *RuntimeDurableStore) PendingEvents(attemptID string) ([]EventSpoolRecord, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
		return nil, err
...
// started to finished.
func (store *RuntimeDurableStore) AppendResult(identity AttemptIdentity, status string, payload json.RawMessage) (ResultSpoolRecord, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
...
func (store *RuntimeDurableStore) StoreResult(record ResultSpoolRecord) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
		return err
...
func (store *RuntimeDurableStore) storeResultLocked(record ResultSpoolRecord) error {
...
func (store *RuntimeDurableStore) PendingResult(attemptID string) (ResultSpoolRecord, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
		return ResultSpoolRecord{}, err
...
func (store *RuntimeDurableStore) AckResult(attemptID, resultID string) error {
	store.mu.Lock()
	defer store.mu.Unlock()
	if err := store.readyLocked(); err != nil {
		return err
...
func (store *RuntimeDurableStore) loadEventSpool() error {
...
func (store *RuntimeDurableStore) loadResultSpool() error {
...
func (store *RuntimeDurableStore) validateEventCoverageLocked(record AssignmentJournalRecord) error {
