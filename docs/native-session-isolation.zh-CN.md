...
- Claude 保留原来的 HOME / CLAUDE_CONFIG_DIR，使用 `--safe-mode --restricted`，
  不使用会跳过订阅认证的 `--bare`。Read/Edit/Write/Glob/Grep 受客户端权限检查；
...
| `CLAUDE_ALLOWED_TOOLS` | 可选 Read、Edit、Write、Glob、Grep、Bash 子集 |
...
检查文件读写工具、Bash、A-B-A、符号链接、跨会话读取、越界写入、环境与父进程
